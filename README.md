...
The software used in the example trial can be found in the GitRepo [YaoGroup/ice_1D_pinn](https://github.com/YaoGroup/ice_1D_pinn/tree/main). A detailed documentation and description of the code mechanism can be found at this link. To conduct more realistic studies involving multiple parameter values and many trials, we recommend directly installing the software package following the instructions in the **ice_1D_pinn** repo.
...
* num_iterations_adam_resampled, num_iterations_adam_fixed, num_iterations_lbfgs _(int)_: Specify the number of iterations to train with each optimizer and collocation method (lines 53-55).
  ...
  * ```lbfgs```: train with L-BFGS optimizer with fixed collocation points.
    ...
    Thus, to train *only* with fixed collocation points, please set ```num_iterations_adam_resampled``` to 0. Conversely, to train *only* with collocation resampling, please set 
